...
class Block {
  ...
  constructor(transactions, previousHash) {
    ...
    this.nonce = 1;
    this.hash = createHash('sha512').update((transactions + previousHash + this.nonce).toString()).digest('hex');
  }
  ...
  calculateHash(nonce) {
    // Your code here
    this.hash = createHash('sha512').update((this.transactions + this.previousHash + nonce).toString()).digest('hex');
    this.nonce = nonce;
